...
def crop(image, target, region):
    ...
    if "keypoints" in target:
        ...
        keypoints = keypoints - paddle.to_tensor([j, i, 0])
...
def hflip(image, target):
    ...
    if "keypoints" in target:
        keypoints = target['keypoints']
        keypoints = keypoints * paddle.to_tensor([-1, 1, 1]) + paddle.to_tensor([w, 0, 0])
...
def resize(image, target, size, max_size=None):
    ...
    if "keypoints" in target:
        keypoints = target["keypoints"]
        scaled_keypoints = keypoints * paddle.to_tensor([ratio_width, ratio_height, 1])
...
def pad(image, target, padding):
    ...
    if "keypoints" in target:
        keypoints = target['keypoints']
        keypoints = keypoints + paddle.to_tensor([padding[0], padding[1], 0])
        target['keypoints'] = keypoints
...
class LargeScaleJitter(object):
    ...
    def __init__(self, output_size=1333, aug_scale_min=0.3, aug_scale_max=2.0):
        self.desired_size = paddle.to_tensor(output_size)
    ...
    def pad_target(self, padding, target):
        target = target.copy()
        if "masks" in target:
            target['masks'] = paddle.nn.functional.pad(target['masks'], (0, padding[1], 0, padding[0]))
        if "keypoints" in target:
            keypoints = target['keypoints']
            keypoints = keypoints + paddle.to_tensor([padding[1], padding[0], 0])
            target['keypoints'] = keypoints
        return target

    def __call__(self, image, target=None):
        image_size = image.size
        image_size = paddle.to_tensor(image_size[::-1])

        out_desired_size = (self.desired_size * image_size / max(image_size)).round().int()

        random_scale = paddle.rand(1) * (self.aug_scale_max - self.aug_scale_min) + self.aug_scale_min
        scaled_size = (random_scale * self.desired_size).round()

        scale = paddle.minimum(scaled_size / image_size[0], scaled_size / image_size[1])
        ...
        if random_scale > 1:
            ...
            offset = (max_offset * paddle.rand(2)).floor().int()
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "keypoints" in target:
            keypoints = target["keypoints"]
            keypoints = keypoints / paddle.to_tensor([w, h, 1], dtype="float32")
            target["keypoints"] = keypoints
        return image, target
...
class GenerateClassificationResults(object):
    ...
    def __call__(self, image, target):
        ...
        multi_label_onehot = paddle.zeros([self.num_cats])
        multi_label_onehot[multi_labels] = 1 # Op `set_value` is executed through `append_op` under the dynamic mode, the corresponding API implementation needs to be upgraded to using `_C_ops` method.
        multi_label_weights = paddle.ones_like(multi_label_onehot)
