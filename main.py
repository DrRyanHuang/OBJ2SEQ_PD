...
def build_optimizer(cfg_train, model_without_ddp, steps_per_epoch=None):
    ...
    if cfg_train.sched == "OneCycle":
        lr_scheduler = paddle.optimizer.lr.OneCycleLR(learning_rate=cfg_train.lr, 
                                                      # steps_per_epoch=steps_per_epoch,
                                                      # epochs=cfg_train.epochs, 
                                                      total_steps = steps_per_epoch * cfg_train.epochs,
                                                      phase_pct=0.2)
    elif cfg_train.sched == "Step":
        lr_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=cfg_train.lr, 
                                                     step_size=cfg_train.lr_drop)
    else:
        raise KeyError
...
def main(args):
    ...
    n_parameters = sum(p.numel() for p in model.parameters() if not p.stop_gradient).cpu().numpy().item()
    ...
    if args.distributed:
        # model = paddle.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
...
if __name__ == '__main__':
    ...
    args.cfg = "configs/detection_r50_seqhead.yaml"
