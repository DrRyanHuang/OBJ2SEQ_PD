...
# import paddle.utils.checkpoint as checkpoint
import numpy as np
...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, axis, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = paddle.static.create_parameter(
                shape=((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads), 
                dtype='float32',
                default_initializer=nn.initializer.TruncatedNormal(mean=0.0, std=.02))
...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.shift_size > 0:
            x = paddle.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            x = shifted_x
    ...
    def forward(self, tensor_list):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                mask = F.interpolate(m[None].float(), size=out.shape[-2:]).to(paddle.bool)[0]
